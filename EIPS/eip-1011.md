...
In addition to block rewards, miners now receive an issuance reward for including successful `vote` transactions into the block on time. This reward is equal to 1/8th that of the reward the validator receives for the successful `vote`. In total, under optimal FFG conditions, miners receive approximately 1/5th of the total ETH issued by the Casper contract.
